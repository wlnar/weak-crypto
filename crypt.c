...
const int LEN = 32;
const int BLOCK = 8;
const int KEY_BIT_LEN = 64;
const int KEY_BYTE_LEN = 8;
  ...
  for (i=0; i<KEY_BYTE_LEN; i=i+BLOCK) {
    for (j=0; j<paddedMessageLen; j=j+BLOCK) {
      for (k=0; k<BLOCK; k++) {
        ...
        printf("[i(%d)_j(%d)_k(%d)_dElem(%d)_kElem(%d)_0x%02X_0x%02X_0x%02X]\n",i,j,k,dElem,kElem,(unsigned char)messageData[dElem],(unsigned char)keyData[kElem],(unsigned char)outData[dElem]);
      }
    }
  }
